defmodule Testly.SplitTests.ConversionReport do
  use Testly.Schema
  ...
  @spec generate_report(Goal.t(), Variation.t()) :: __MODULE__.t()
  ...
  def put_rates_by_date_reports(%__MODULE__{} = report, goal, variation, from_date, to_date) do
    ...
    visits_by_date =
      variation.visits
      |> Enum.group_by(&DateTime.to_date(&1.session_recording.created_at))
      |> Enum.into(%{})
  ...
  def put_is_winner(reports, true = _is_finished) do
    ...
    for report <- reports do
      %__MODULE__{report | is_winner: is_any_winner && report == winner}
    end
  end

  def put_is_winner(reports, false = _is_finished) do
    for report <- reports do
      %__MODULE__{report | is_winner: false}
    end
  end
  ...
  defp calc_improvement_rate(%__MODULE__{control: true}, _control_report), do: nil

  defp calc_improvement_rate(control_report, %__MODULE__{conversion_rate: 0} = report),
    do: calc_improvement_rate(control_report, %__MODULE__{report | conversion_rate: 0.0})

  defp calc_improvement_rate(control_report, %__MODULE__{conversion_rate: 0.0}),
    do: control_report.conversion_rate * 1000 / 1
